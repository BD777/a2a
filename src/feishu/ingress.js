import { eventRoot, extractEventText, senderId } from './message-parser.js';

export class FeishuIngress {
  constructor({ clients, scheduler, config, logger }) {
  ...
  async handleEvent(appId, data, source) {
    ...
    this.logger.info(`ingress ${source} chat=${record.chatId} root=${record.rootMessageId} msg=${record.messageId} chars=${record.text.length}`);
